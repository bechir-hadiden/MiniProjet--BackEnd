...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.cors(Customizer.withDefaults()); // Appliquer la configuration CORS
	    ...
	    http.cors(cors -> cors.configurationSource(new CorsConfigurationSource() {
	        ...
	        public CorsConfiguration getCorsConfiguration(HttpServletRequest request) {
	        	 CorsConfiguration cors = new CorsConfiguration();
	        	    cors.setAllowedOrigins(Collections.singletonList("http://localhost:4200")); // Autoriser Angular
	        	    cors.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS")); // Méthodes HTTP permises
	        	    cors.setAllowedHeaders(Arrays.asList("Authorization", "Content-Type")); // En-têtes acceptés
	        	    cors.setExposedHeaders(Collections.singletonList("Authorization")); // Exposer l'en-tête Authorization
	        	    cors.setAllowCredentials(true); // Autoriser les cookies si nécessaire
	    ...
	    http.authorizeHttpRequests(authorize -> authorize
	            ...
	            .requestMatchers(HttpMethod.POST, "/api/addDepar/**").hasAuthority("ADMIN")
	            ...
	            .requestMatchers("/college/**").hasAnyAuthority("ADMIN", "USER")
